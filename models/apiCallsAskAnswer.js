...
function getSpecificQuestions (questionIDs, callBack, options) {
	...
	var url = questionAnswerAPICall(settings);
	console.log(url);
	$.ajax({
		type: "GET",
		url: url,
...
function questionAnswerAPICall (options) {
	...
	var url = "http://" + defaultSettings["URL"]["CustomerName"] + "." + defaultSettings["URL"]["BaseURL"] + "questions." + defaultSettings["URL"]["Format"] + "?apiversion=" + defaultSettings["URL"]["ApiVersion"] + "&passkey=" + defaultSettings["URL"]["PassKey"];
